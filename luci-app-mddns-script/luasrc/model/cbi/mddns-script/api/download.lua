...
-- 已下完→success; 进程在跑→downloading; 没跑→启动+downloading
-- client_total: 前端首次拿到 total 后回传, 命中则跳过 HEAD, 避免每 2s 轮询都请求
function download(bin_path, bin_url, client_total)
	...
	-- 2. 文件不存在: 无下载进程才启动 (前端每 2s 轮询; 首次启动后后续轮询 dl_running=true 跳过, 防重复起 wget)
	...
	-- 4. 下载中 (刚启动 或 上一次轮询已在跑); total 回传前端缓存
