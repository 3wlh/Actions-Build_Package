...
IPK_url="$(echo "${Data}" | grep -Eo '"browser_download_url":\s*".*'${FILE}_'.*_'${PACKAGES_ARCH}'.*"' | cut -d '"' -f 4)"
...
curl -s https://api.github.com/repos/3wlh/OpenWrt_Packages/releases/tags/GitHub-Actions_luci-app-mddns-script | grep -Eo '"browser_download_url":\s*".*luci-app-mddns-script.*all.*"' | cut -d '"' -f 4
